...
def down_sample_for_g_input(high_res_3_phase):
    """
    :return: a down-sample of the grey material.
    """
    # first choose the grey phase in the image:
    grey_material = torch.index_select(high_res_3_phase, 1, grey_index)
    # down sample:
...
def up_sample_for_similarity_check(low_res_im):
    """
    ...
    """
    grey_material = torch.index_select(low_res_im, 1, grey_index)
    up_sample = torch.nn.Upsample(scale_factor=up_sample_factor,
                                  mode='bilinear')
    return up_sample(grey_material)


def pixel_wise_distance(low_res_im, generated_high_res):
    """
    ...
    """
    generated_grey = torch.index_select(generated_high_res, 1, grey_index)
    up_sample = up_sample_for_similarity_check(low_res_im)
